...
1.  在服务器列表最左侧的“选择”列中勾选需要同步的服务器（可多选）。
2.  如需全选，点击上方工具栏的 **✅ 全选** 按钮。
3.  点击 **▶️ 开始监控**：仅对已勾选的服务器启动监控与同步。
4.  程序将为每个已勾选的服务器启动独立的监控进程，互不影响。
5.  您可以在“状态”列查看每个任务的运行情况（就绪、监控中、错误）。
6.  如需停止，点击 **⏸️ 停止监控**（停止所有正在监控的服务器）。
...
*   **代理兼容性**: 程序已内置**完整的代理绕过功能**，即使系统开启了代理软件（如Clash、V2Ray等），也能正常连接FTP服务器。所有FTP连接（包括控制通道和数据通道）都会自动使用直连方式，完全不受系统代理影响。这解决了FTP被动模式（PASV）在代理环境下无法传输数据的问题。
